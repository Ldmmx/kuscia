...
function test_runc_kuscia_images() {
  ...
  local get_pull_image_tag=$(get_images "${TEST_IMAGE_NAME}" "${LITE_ALICE_CONTAINER}" )
  ...
  local get_rm_image_tag=$(get_images "${TEST_LOAD_IMAGE_TAG_NAME}" "${LITE_ALICE_CONTAINER}")
  ...
  local get_load_image_tag=$(get_images "${TEST_LOAD_IMAGE_TAG_NAME}" "${LITE_ALICE_CONTAINER}")
  ...
  local get_tag_image_tag=$(get_images "${GET_IMAGE_TAG_NAME}" "${LITE_ALICE_CONTAINER}")
...
function test_runp_kuscia_images() {
  ...
  local get_pull_image_tag=$(get_images "${TEST_IMAGE_NAME}" "${LITE_ALICE_CONTAINER}" "${RUNTIME_RUNP}")
  ...
  local get_load_image_tag=$(get_images "${TEST_LOAD_IMAGE_TAG_NAME}" "${LITE_ALICE_CONTAINER}" "${RUNTIME_RUNP}")
  ...
  local get_tag_image_tag=$(get_images "${GET_IMAGE_TAG_NAME}" "${LITE_ALICE_CONTAINER}" "${RUNTIME_RUNP}")
  assertEquals "kuscia tag images runp success" "${GET_IMAGE_TAG_NAME}" "${get_tag_image_tag}"

  #test builtin image
  local orign_image_ID=$(get_image_ID "${TEST_BUILTIN_IMAGE}" "${LITE_ALICE_CONTAINER}" "${RUNTIME_RUNP}")
  docker exec -i "${LITE_ALICE_CONTAINER}" bash -c "kuscia images --runtime=runp builtin ${TEST_IMAGE_NAME}"
  local builtin_image_ID=$(get_image_ID "${TEST_BUILTIN_IMAGE}" "${LITE_ALICE_CONTAINER}" "${RUNTIME_RUNP}")
  ...
  local get_rm_image_tag=$(get_images "${TEST_LOAD_IMAGE_TAG_NAME}" "${LITE_ALICE_CONTAINER}" "${RUNTIME_RUNP}")
