	...
	for _, c := range pod.Spec.Containers {
		...
		if c.StartupProbe != nil {
			...
			if _, ok := m.workers[key]; ok {
				nlog.Warnf("Startup probe already exists for container, pod=%v, containerName=%v", format.Pod(pod), c.Name)
				return
			}
		...
		if c.ReadinessProbe != nil {
			...
			if _, ok := m.workers[key]; ok {
				nlog.Warnf("Readiness probe already exists for container, pod=%v, containerName=%v", format.Pod(pod), c.Name)
				return
			}
		...
		if c.LivenessProbe != nil {
			...
			if _, ok := m.workers[key]; ok {
				nlog.Warnf("Liveness probe already exists for container, pod=%v, containerName=%v", format.Pod(pod), c.Name)
				return
			}
...
func (m *manager) UpdatePodStatus(podUID types.UID, podStatus *v1.PodStatus) {
	for i, c := range podStatus.ContainerStatuses {
		...
		if c.State.Running == nil {
			started = false
		} else if result, ok := m.startupManager.Get(pkgcontainer.ParseContainerID(c.ContainerID)); ok {
		...
		if started {
			...
			if c.State.Running == nil {
				ready = false
			} else if result, ok := m.readinessManager.Get(pkgcontainer.ParseContainerID(c.ContainerID)); ok && result == results.Success {
				ready = true
			} else {
				// The check whether there is a probe which hasn't run yet.
				...
				if exists {
					...
					default: // Non-blocking.
						nlog.Infof("Failed to trigger a manual run, probe=%v", w.probeType.String())
					}
				}
			}
