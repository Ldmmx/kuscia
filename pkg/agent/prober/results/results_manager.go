...
type manager struct {
	...
	cache map[pkgcontainer.CtrID]Result
	// channel of updates
	updates chan Update
}
...
func NewManager() Manager {
	return &manager{
		cache:   make(map[pkgcontainer.CtrID]Result),
		updates: make(chan Update, 20),
	}
}

func (m *manager) Get(id pkgcontainer.CtrID) (Result, bool) {
	m.RLock()
	defer m.RUnlock()
...
func (m *manager) Set(id pkgcontainer.CtrID, result Result, pod *v1.Pod) {
...
func (m *manager) setInternal(id pkgcontainer.CtrID, result Result) bool {
	m.Lock()
	defer m.Unlock()
...
func (m *manager) Remove(id pkgcontainer.CtrID) {
	m.Lock()
	defer m.Unlock()
	delete(m.cache, id)
}
