...
func TestDoProbe(t *testing.T) {
	...
	for _, probeType := range [...]probeType{liveness, readiness, startup} {
		...
		for i, test := range tests {
			...
			m.prober.tcp = fakeTCPProber{probe.Success, nil}
			...
			if c := w.doProbe(context.Background()); c != test.expectContinue[probeType.String()] {
			...
			// Clean up.
			m.statusManager = status.NewManager(&fake.Clientset{}, pkgpod.NewBasicPodManager(nil), &statustest.FakePodDeletionSafetyProvider{})
...
func TestFailureThreshold(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		m.prober.tcp = fakeTCPProber{probe.Failure, nil}
...
func expectResult(t *testing.T, w *worker, expectedResult results.Result, msg string) {
	...
	if !ok {
		t.Fatalf("[%s - %s] Expected result to be set, but was not set", w.probeType, msg)
	}

	assert.Equal(t, expectedResult, result, msg)
}
