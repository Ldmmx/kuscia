...
func topkgcontainerImageSpec(image *runtimeapi.Image) pkgcontainer.ImageSpec {
	var annotations []pkgcontainer.Annotation
	...
	if image.Spec != nil && len(image.Spec.Annotations) > 0 {
		...
		for _, k := range annotationKeys {
			annotations = append(annotations, pkgcontainer.Annotation{
				Name:  k,
	...
	return pkgcontainer.ImageSpec{
		Image:       image.Id,
		Annotations: annotations,
	}
}

func toRuntimeAPIImageSpec(imageSpec pkgcontainer.ImageSpec) *runtimeapi.ImageSpec {
