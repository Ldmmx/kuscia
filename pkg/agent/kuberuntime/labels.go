...
func newContainerAnnotations(container *v1.Container, pod *v1.Pod, restartCount int, opts *pkgcontainer.RunContainerOptions) map[string]string {
	annotations := map[string]string{}
	...
	annotations[containerHashLabel] = strconv.FormatUint(pkgcontainer.HashContainer(container), 16)
	...
	if container.Lifecycle != nil && container.Lifecycle.PreStop != nil {
		...
		if err != nil {
			nlog.Errorf("Unable to marshal lifecycle PreStop handler for container, containerName=%v, pod=%v, detail-> %v", container.Name, format.Pod(pod), err)
		} else {
	...
	if len(container.Ports) > 0 {
		...
		if err != nil {
			nlog.Errorf("Unable to marshal container ports for container, containerName=%v, pod=%v, detail-> %v", container.Name, format.Pod(pod), err)
		} else {
...
func getContainerInfoFromAnnotations(annotations map[string]string) *annotatedContainerInfo {
	...
	if containerInfo.Hash, err = getUint64ValueFromLabel(annotations, containerHashLabel); err != nil {
		nlog.Errorf("Unable to get label value from annotations, label=%v, annotations=%v", containerHashLabel, annotations)
	}
	if containerInfo.RestartCount, err = getIntValueFromLabel(annotations, containerRestartCountLabel); err != nil {
		nlog.Errorf("Unable to get label value from annotations, label=%v, annotations=%v", containerRestartCountLabel, annotations)
	}
	if containerInfo.PodDeletionGracePeriod, err = getInt64PointerFromLabel(annotations, podDeletionGracePeriodLabel); err != nil {
		nlog.Errorf("Unable to get label value from annotations, label=%v, annotations=%v", podDeletionGracePeriodLabel, annotations)
	}
	if containerInfo.PodTerminationGracePeriod, err = getInt64PointerFromLabel(annotations, podTerminationGracePeriodLabel); err != nil {
		nlog.Errorf("Unable to get label value from annotations, label=%v, annotations=%v", podTerminationGracePeriodLabel, annotations)
	}

	preStopHandler := &v1.LifecycleHandler{}
	if found, err := getJSONObjectFromLabel(annotations, containerPreStopHandlerLabel, preStopHandler); err != nil {
		nlog.Errorf("Unable to get label value from annotations, label=%v, annotations=%v", containerPreStopHandlerLabel, annotations)
	} else if found {
	...
	if found, err := getJSONObjectFromLabel(annotations, containerPortsLabel, &containerPorts); err != nil {
		nlog.Errorf("Unable to get label value from annotations, label=%v, annotations=%v", containerPortsLabel, annotations)
	} else if found {
...
func getStringValueFromLabel(labels map[string]string, label string) string {
	...
	nlog.Warnf("Container doesn't have requested label %q, it may be an old or invalid container", label)
