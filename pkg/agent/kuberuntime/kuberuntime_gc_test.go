...
func TestContainerGC(t *testing.T) {
	...
	podStateProvider := m.containerGC.podStateProvider.(*fakePodStateProvider)
	defaultGCPolicy := pkgcontainer.GCPolicy{MinAge: time.Hour, MaxPerPodContainer: 2, MaxContainers: 6}

	for _, test := range []struct {
		description          string                 // description of the test case
		containers           []containerTemplate    // templates of containers
		policy               *pkgcontainer.GCPolicy // container gc policy
	...
	}{
		{
			...
			policy:               &pkgcontainer.GCPolicy{MinAge: time.Minute, MaxPerPodContainer: 1, MaxContainers: 0},
			remain:               []int{},
			evictTerminatingPods: false,
		...
		},
		{
			...
			policy:               &pkgcontainer.GCPolicy{MinAge: time.Minute, MaxPerPodContainer: -1, MaxContainers: 4},
			remain:               []int{0, 1, 2, 3},
			evictTerminatingPods: false,
			allSourcesReady:      true,
		},
		{
			...
			policy:               &pkgcontainer.GCPolicy{MinAge: time.Minute, MaxPerPodContainer: -1, MaxContainers: -1},
			remain:               []int{0, 1, 2},
			evictTerminatingPods: false,
			allSourcesReady:      true,
		},
...
func TestPodLogDirectoryGC(t *testing.T) {
	...
	removed := []string{
		filepath.Join(m.podStdoutRootDirectory, "789"),
		filepath.Join(m.podStdoutRootDirectory, "012"),
		filepath.Join(m.podStdoutRootDirectory, "name_namespace_654"),
	}
...
func TestUnknownStateContainerGC(t *testing.T) {
	...
	// podStateProvider := m.containerGC.podStateProvider.(*fakePodStateProvider)
	defaultGCPolicy := pkgcontainer.GCPolicy{MinAge: time.Hour, MaxPerPodContainer: 0, MaxContainers: 0}
