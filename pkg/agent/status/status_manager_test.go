...
func newTestManager(kubeClient clientset.Interface) *manager {
	podManager := pkgpod.NewBasicPodManager(podtest.NewFakeMirrorClient())
...
func TestStatusNormalizationEnforcesMaxBytes(t *testing.T) {
	...
	if count > pkgcontainer.MaxPodTerminationMessageLogLength {
...
func TestSetContainerReadiness(t *testing.T) {
	cID1 := pkgcontainer.CtrID{Type: "test", ID: "1"}
	cID2 := pkgcontainer.CtrID{Type: "test", ID: "2"}
	...
	m.SetContainerReadiness(pod.UID, pkgcontainer.CtrID{Type: "test", ID: "foo"}, true)
	verifyUpdates(t, m, 0)
	status = expectPodStatus(t, m, pod)
...
func TestSetContainerStartup(t *testing.T) {
	...
	m.SetContainerStartup(pod.UID, pkgcontainer.CtrID{Type: "test", ID: "foo"}, true)
	verifyUpdates(t, m, 0)
	status = expectPodStatus(t, m, pod)
