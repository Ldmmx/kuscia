...
type Manager interface {
	...
	SetContainerReadiness(podUID types.UID, containerID pkgcontainer.CtrID, ready bool)

	// SetContainerStartup updates the cached container status with the given startup, and
	// triggers a status update.
	SetContainerStartup(podUID types.UID, containerID pkgcontainer.CtrID, started bool)
...
func NewManager(kubeClient clientset.Interface, podManager pkgpod.Manager, podDeletionSafety PodDeletionSafetyProvider) Manager {
	return &manager{
...
func (m *manager) Start() {
	...
	if m.kubeClient == nil {
		nlog.Infof("Kubernetes client is nil, not starting status manager")
		return
	}

	nlog.Infof("Starting to sync pod status with apiserver")
	...
	go wait.Forever(func() {
		for {
			...
			case syncRequest := <-m.podStatusChannel:
				nlog.Debugf("Status Manager: syncing pod with status from podStatusChannel, podUID=%v, statusVersion=%v, status=%v",
...
func (m *manager) SetContainerReadiness(podUID types.UID, containerID pkgcontainer.CtrID, ready bool) {
	m.podStatusesLock.Lock()
	defer m.podStatusesLock.Unlock()

	pod, ok := m.podManager.GetPodByUID(podUID)
	if !ok {
		nlog.Debugf("Pod %q has been deleted, no need to update readiness", string(podUID))
		return
	}

	oldStatus, found := m.podStatuses[pod.UID]
	if !found {
		nlog.Infof("Container readiness changed before pod has synced, pod=%v, containerID=%v", format.Pod(pod), containerID.String())
		return
	}
	...
	if !ok {
		nlog.Infof("Container readiness changed for unknown container, pod=%v, containerID=%v", format.Pod(pod), containerID.String())
		return
	}

	if containerStatus.Ready == ready {
		nlog.Debugf("Container readiness unchanged, ready=%v, pod=%v, containerID=%v", ready, format.Pod(pod), containerID.String())
		return
	}
	...
	updateConditionFunc := func(conditionType v1.PodConditionType, condition v1.PodCondition) {
		...
		if conditionIndex != -1 {
			...
			nlog.Infof("PodStatus missing condition type, conditionType=%v, status=%v", conditionType, status)
...
func (m *manager) SetContainerStartup(podUID types.UID, containerID pkgcontainer.CtrID, started bool) {
	m.podStatusesLock.Lock()
	defer m.podStatusesLock.Unlock()

	pod, ok := m.podManager.GetPodByUID(podUID)
	if !ok {
		nlog.Debugf("Pod %q has been deleted, no need to update startup", string(podUID))
		return
	}

	oldStatus, found := m.podStatuses[pod.UID]
	if !found {
		nlog.Infof("Container startup changed before pod has synced, pod=%v, containerID=%v", format.Pod(pod), containerID.String())
		return
	}
	...
	containerStatus, _, ok := findContainerStatus(&oldStatus.status, containerID.String())
	if !ok {
		nlog.Infof("Container startup changed for unknown container, pod=%v, containerID=%v", format.Pod(pod), containerID.String())
		return
	}

	if containerStatus.Started != nil && *containerStatus.Started == started {
		nlog.Debugf("Container startup unchanged, pod=%v, containerID=%v", format.Pod(pod), containerID.String())
		return
	}
...
func (m *manager) TerminatePod(pod *v1.Pod) {
	...
	nlog.Debugf("TerminatePod calling updateStatusInternal, pod=%v", format.Pod(pod))
...
func (m *manager) updateStatusInternal(pod *v1.Pod, status v1.PodStatus, forceUpdate bool) bool {
	...
	if err := checkContainerStateTransition(oldStatus.ContainerStatuses, status.ContainerStatuses, pod.Spec.RestartPolicy); err != nil {
		nlog.Errorf("Status update on pod %q aborted: %v", format.Pod(pod), err)
		return false
	}
	...
	case m.podStatusChannel <- podStatusSyncRequest{pod.UID, newStatus}:
		nlog.Debugf("Status Manager: adding pod %q with new status to podStatusChannel, statusVersion=%v, status=%v",
			format.Pod(pod), newStatus.version, newStatus.status)
		return true
	default:
		// Let the periodic syncBatch handle the update if the channel is full.
		// We can't block, since we hold the mutex lock.
		nlog.Debugf("Skipping the status update for pod %q for now because the channel is full, status=%v", format.Pod(pod), status)
		return false
	}
}
...
func (m *manager) RemoveOrphanedStatuses(podUIDs map[types.UID]bool) {
	...
	for key := range m.podStatuses {
		if _, ok := podUIDs[key]; !ok {
			nlog.Debugf("Removing pod %q from status map.", key)
...
func (m *manager) syncBatch() {
	...
	func() { // Critical section
		...
		for uid, status := range m.podStatuses {
			...
			if mirrorUID, ok := podToMirror[kubetypes.ResolvedPodUID(uid)]; ok {
				if mirrorUID == "" {
					nlog.Debugf("Static pod %q does not have a corresponding mirror pod; skipping", format.PodDesc(status.podName, status.podNamespace, uid))
					continue
				}
				syncedUID = mirrorUID
			}
	...
	for _, update := range updatedStatuses {
		nlog.Debugf("Status Manager: sync pod %q in syncbatch", update.podUID)
...
func (m *manager) syncPod(uid types.UID, status versionedPodStatus) {
	if !m.needsUpdate(uid, status) {
		nlog.Infof("Status for pod %q is up-to-date; skipping", uid)
		return
	}
	...
	if k8serrors.IsNotFound(err) {
		nlog.Infof("Pod %q does not exist on the server", format.PodDesc(status.podName, status.podNamespace, uid))
		// If the Pod is deleted the status will be cleared in
		// RemoveOrphanedStatuses, so we just ignore the update here.
		return
	}
	if err != nil {
		nlog.Errorf("Failed to get status for pod %q: %v", format.PodDesc(status.podName, status.podNamespace, uid), err)
		return
	}
	...
	if len(translatedUID) > 0 && translatedUID != kubetypes.ResolvedPodUID(uid) {
		nlog.Infof("Pod %q was deleted and then recreated, skipping status update, oldPodUID=%v, podUID=%v", format.Pod(pod), uid, translatedUID)
	...
	newPod, patchBytes, unchanged, err := statusutil.PatchPodStatus(context.Background(), m.kubeClient, pod.Namespace, pod.Name, pod.UID, pod.Status, mergedStatus)
	nlog.Infof("Patch status for pod %q, patch=%v", format.Pod(pod), string(patchBytes))

	if err != nil {
		nlog.Errorf("Failed to update status for pod %q: %v", format.Pod(pod), err)
		return
	}
	if unchanged {
	...
	if m.canBeDeleted(pod, status.status) {
		...
		if err != nil {
			nlog.Infof("Failed to delete status for pod %q: %v", format.Pod(pod), err)
			return
		}
		nlog.Infof("Pod %q fully terminated and removed from etcd", format.Pod(pod))
		m.deletePodStatus(uid)
	}
}
...
func (m *manager) needsReconcile(uid types.UID, status v1.PodStatus) bool {
	...
	pod, ok := m.podManager.GetPodByUID(uid)
	if !ok {
		nlog.Debugf("Pod %q has been deleted, no need to reconcile", string(uid))
		return false
	}
	...
	if kubetypes.IsStaticPod(pod) {
		...
		if !ok {
			nlog.Debugf("Static pod %q has no corresponding mirror pod, no need to reconcile", format.Pod(pod))
			return false
		}
		pod = mirrorPod
	}
	...
	nlog.Infof("Pod status is inconsistent with cached status for pod %q, a reconciliation should be triggered, statusDiff=%v", format.Pod(pod), cmp.Diff(podStatus, &status))
...
func normalizeStatus(pod *v1.Pod, status *v1.PodStatus) *v1.PodStatus {
	bytesPerStatus := pkgcontainer.MaxPodTerminationMessageLogLength
