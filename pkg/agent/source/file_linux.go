...
func (s *sourceFile) produceWatchEvent(e *fsnotify.Event) error {
	...
	if strings.HasPrefix(filepath.Base(e.Name), ".") {
		nlog.Debugf("Ignored pod manifest, because it starts with dots, eventName=%v", e.Name)
		return nil
	}
	...
	nlog.Debugf("Produce watch event: %v, filename=%v", eventType, e.Name)
...
func (s *sourceFile) consumeWatchEvent(e *watchEvent) error {
	...
	case podAdd, podModify:
		...
		nlog.Infof("Receive pod %q add or update event from file %q", format.Pod(pod), e.fileName)
	...
	case podDelete:
		if objKey, keyExist := s.fileKeyMapping[e.fileName]; keyExist {
			...
			if err != nil {
				...
				nlog.Infof("Receive pod %q delete event from file %q", objKey, e.fileName)
