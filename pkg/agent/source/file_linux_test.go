...
func TestExtractFromNonExistentFile(t *testing.T) {
	...
	lw := newSourceFile(createFileSourceCfg("test-namespace", "localhost", "/some/fake/file", time.Millisecond), ch)
	err := lw.doWatch()
	if err == nil {
...
func TestUpdateOnNonExistentFile(t *testing.T) {
	...
	lw := newSourceFile(createFileSourceCfg("test-namespace", "localhost", "random_non_existent_path", time.Millisecond), ch)
	lw.run()
	select {
	case update := <-ch:
...
func TestReadPodsFromFileExistAlready(t *testing.T) {
	...
	for _, testCase := range testCases {
		func() {
			...
			ch := make(chan kubetypes.PodUpdate)
			lw := newSourceFile(createFileSourceCfg("test-namespace", hostname, file, time.Millisecond), ch)
			lw.run()
...
func watchFileAdded(watchDir bool, symlink bool, t *testing.T) {
	...
	for index, testCase := range testCases {
		func() {
			...
			var lw *sourceFile
			if watchDir {
				lw = newSourceFile(createFileSourceCfg("test-namespace", hostname, dirName, 100*time.Millisecond), ch)
			} else {
				lw = newSourceFile(createFileSourceCfg("test-namespace", hostname, filepath.Join(dirName, fileName), 100*time.Millisecond), ch)
			}
			lw.run()
...
func watchFileChanged(watchDir bool, symlink bool, period time.Duration, t *testing.T) {
	...
	for index, testCase := range testCases {
		func() {
			...
			var lw *sourceFile
			if watchDir {
				lw = newSourceFile(createFileSourceCfg("test-namespace", hostname, dirName, period), ch)
			} else {
				lw = newSourceFile(createFileSourceCfg("test-namespace", hostname, file, period), ch)
			}
			lw.run()
...
func expectUpdate(t *testing.T, ch chan kubetypes.PodUpdate, testCase *testCase) {
...
func expectEmptyUpdate(t *testing.T, ch chan kubetypes.PodUpdate) {
	timer := time.After(5 * time.Second)
	for {
		select {
		case update := <-ch:
