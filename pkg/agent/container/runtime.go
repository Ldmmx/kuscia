...
func BuildContainerID(typ, ID string) CtrID {
	return CtrID{Type: typ, ID: ID}
}

// ParseContainerID is a convenience method for creating a CtrID from an ID string.
func ParseContainerID(containerID string) CtrID {
	var id CtrID
	if err := id.ParseString(containerID); err != nil {
		nlog.Errorf("Parsing containerID failed: %v", err)
	}
	return id
}

// ParseString converts given string into CtrID
func (c *CtrID) ParseString(data string) error {
...
func (c *CtrID) String() string {
	return fmt.Sprintf("%s://%s", c.Type, c.ID)
}

// IsEmpty returns whether given CtrID is empty.
func (c *CtrID) IsEmpty() bool {
	return *c == CtrID{}
}

// MarshalJSON formats a given CtrID into a byte array.
func (c *CtrID) MarshalJSON() ([]byte, error) {
	return []byte(fmt.Sprintf("%q", c.String())), nil
}

// UnmarshalJSON parses CtrID from a given array of bytes.
func (c *CtrID) UnmarshalJSON(data []byte) error {
...
func (id DockerID) ContainerID() CtrID {
	return CtrID{
		Type: "docker",
		ID:   string(id),
	}
...
func (p *Pod) FindContainerByID(id CtrID) *Container {
	for _, c := range p.Containers {
		if c.ID == id {
			return c
		}
...
func (p *Pod) FindSandboxByID(id CtrID) *Container {
