...
func ShouldContainerBeRestarted(container *v1.Container, pod *v1.Pod, podStatus *PodStatus) bool {
	...
	if pod.Spec.RestartPolicy == v1.RestartPolicyNever {
		nlog.Debugf("Already ran container, do nothing, pod=%v, containerName=%v", format.Pod(pod), container.Name)
		return false
	}
	if pod.Spec.RestartPolicy == v1.RestartPolicyOnFailure {
		// Check the exit code.
		if status.ExitCode == 0 {
			nlog.Debugf("Already successfully ran container, do nothing, pod=%v, containerName=%v", format.Pod(pod), container.Name)
			return false
		}
	}
	return true
...
func ConvertPodStatusToRunningPod(runtimeName string, podStatus *PodStatus) Pod {
	...
	// Populate sandboxes in pkgcontainer.Pod
	for _, sandbox := range podStatus.SandboxStatuses {
		runningPod.Sandboxes = append(runningPod.Sandboxes, &Container{
			ID:    CtrID{Type: runtimeName, ID: sandbox.Id},
...
func MakePortMappings(container *v1.Container) (ports []PortMapping) {
	...
	for _, p := range container.Ports {
		...
		if _, ok := names[name]; ok {
			nlog.Infof("Port name %q conflicted, it is defined more than once", name)
			continue
		}
