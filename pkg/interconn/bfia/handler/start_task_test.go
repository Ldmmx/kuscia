// Copyright 2023 Ant Group Co., Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//   http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package handler

import (
	"context"
	"net/http"
	"reflect"
	"testing"

	"github.com/stretchr/testify/assert"

	"github.com/secretflow/kuscia/pkg/crd/apis/kuscia/v1alpha1"
	kusciaclientsetfake "github.com/secretflow/kuscia/pkg/crd/clientset/versioned/fake"
	kusciainformers "github.com/secretflow/kuscia/pkg/crd/informers/externalversions"
	"github.com/secretflow/kuscia/pkg/web/api"
	"github.com/secretflow/kuscia/pkg/web/errorcode"
	"github.com/secretflow/kuscia/proto/api/v1/interconn"
)

func TestNewStartTaskHandler(t *testing.T) {
	ctx := context.Background()
	kusciaFakeClient := kusciaclientsetfake.NewSimpleClientset()
	rm, _ := NewResourcesManager(ctx, kusciaFakeClient)

	tests := []struct {
		name       string
		wantNotNil bool
	}{
		{
			name:       "new start task handler",
			wantNotNil: true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.wantNotNil, NewStartTaskHandler(rm) != nil)
		})
	}
}

func Test_startTaskHandler_GetType(t *testing.T) {
	ctx := context.Background()
	kusciaFakeClient := kusciaclientsetfake.NewSimpleClientset()
	rm, _ := NewResourcesManager(ctx, kusciaFakeClient)
	h := NewStartTaskHandler(rm)

	tests := []struct {
		name         string
		wantReqType  reflect.Type
		wantRespType reflect.Type
	}{
		{
			name:         "get req and resp type",
			wantReqType:  reflect.TypeOf(interconn.StartTaskRequest{}),
			wantRespType: reflect.TypeOf(interconn.CommonResponse{}),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotReqType, gotRespType := h.GetType()
			assert.Equal(t, tt.wantReqType, gotReqType)
			assert.Equal(t, tt.wantRespType, gotRespType)
		})
	}
}

func Test_startTaskHandler_Handle(t *testing.T) {
	kj := makeKusciaJob("job-1", nil, v1alpha1.JobStartStage)

	kusciaFakeClient := kusciaclientsetfake.NewSimpleClientset(kj)
	kusciaInformerFactory := kusciainformers.NewSharedInformerFactory(kusciaFakeClient, 0)
	ktInformer := kusciaInformerFactory.Kuscia().V1alpha1().KusciaTasks()
	kjInformer := kusciaInformerFactory.Kuscia().V1alpha1().KusciaJobs()

	kjInformer.Informer().GetStore().Add(kj)

	rm := &ResourcesManager{
		KusciaClient: kusciaFakeClient,
		KtLister:     ktInformer.Lister(),
		KjLister:     kjInformer.Lister(),
		jobTaskInfo:  make(map[string]map[string]struct{}),
		taskJobInfo:  make(map[string]string),
	}
	h := NewStartTaskHandler(rm)

	type args struct {
		ctx     *api.BizContext
		request api.ProtoRequest
	}

	tests := []struct {
		name     string
		args     args
		wantCode int32
	}{
		{
			name: "req job id doesn't exist",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					TaskId:   "task-2",
					JobId:    "job-2",
					TaskName: "sf",
				},
			},
			wantCode: http.StatusBadRequest,
		},
		{
			name: "req task id doesn't exist",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					TaskId:   "task-2",
					JobId:    "job-1",
					TaskName: "sf",
				},
			},
			wantCode: http.StatusBadRequest,
		},
		{
			name: "set task id succeeded",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					TaskId:   "task-1",
					JobId:    "job-1",
					TaskName: "hetero_logistic_regression_1",
				},
			},
			wantCode: http.StatusOK,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp := h.Handle(tt.args.ctx, tt.args.request)
			got := resp.(*interconn.CommonResponse)
			assert.Equal(t, tt.wantCode, got.Code)
		})
	}
}

func Test_startTaskHandler_Validate(t *testing.T) {
	ctx := context.Background()
	kusciaFakeClient := kusciaclientsetfake.NewSimpleClientset()
	rm, _ := NewResourcesManager(ctx, kusciaFakeClient)
	h := NewStartTaskHandler(rm)

	type args struct {
		ctx     *api.BizContext
		request api.ProtoRequest
		errs    *errorcode.Errs
	}

	tests := []struct {
		name    string
		args    args
		wantErr bool
	}{
		{
			name: "req type is invalid",
			args: args{
				ctx:     nil,
				request: &interconn.StartJobRequest{},
				errs:    &errorcode.Errs{},
			},
			wantErr: true,
		},
		{
			name: "req job id is empty",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					TaskId:   "task-1",
					TaskName: "hetero_logistic_regression_1",
				},
				errs: &errorcode.Errs{},
			},
			wantErr: true,
		},
		{
			name: "req task id is empty",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					JobId:    "job-1",
					TaskName: "hetero_logistic_regression_1",
				},
				errs: &errorcode.Errs{},
			},
			wantErr: true,
		},
		{
			name: "req task name is empty",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					JobId:  "job-1",
					TaskId: "task-1",
				},
				errs: &errorcode.Errs{},
			},
			wantErr: true,
		},
		{
			name: "req is valid",
			args: args{
				ctx: nil,
				request: &interconn.StartTaskRequest{
					JobId:    "job-1",
					TaskId:   "task-1",
					TaskName: "hetero_logistic_regression_1",
				},
				errs: &errorcode.Errs{},
			},
			wantErr: false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h.Validate(tt.args.ctx, tt.args.request, tt.args.errs)
			assert.Equal(t, tt.wantErr, len(*tt.args.errs) != 0)
		})
	}
}
