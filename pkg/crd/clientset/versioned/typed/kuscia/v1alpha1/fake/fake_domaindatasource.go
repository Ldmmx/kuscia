// Copyright 2023 Ant Group Co., Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//   http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "github.com/secretflow/kuscia/pkg/crd/apis/kuscia/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDomainDataSources implements DomainDataSourceInterface
type FakeDomainDataSources struct {
	Fake *FakeKusciaV1alpha1
	ns   string
}

var domaindatasourcesResource = schema.GroupVersionResource{Group: "kuscia.secretflow", Version: "v1alpha1", Resource: "domaindatasources"}

var domaindatasourcesKind = schema.GroupVersionKind{Group: "kuscia.secretflow", Version: "v1alpha1", Kind: "DomainDataSource"}

// Get takes name of the domainDataSource, and returns the corresponding domainDataSource object, and an error if there is any.
func (c *FakeDomainDataSources) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DomainDataSource, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(domaindatasourcesResource, c.ns, name), &v1alpha1.DomainDataSource{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataSource), err
}

// List takes label and field selectors, and returns the list of DomainDataSources that match those selectors.
func (c *FakeDomainDataSources) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DomainDataSourceList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(domaindatasourcesResource, domaindatasourcesKind, c.ns, opts), &v1alpha1.DomainDataSourceList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DomainDataSourceList{ListMeta: obj.(*v1alpha1.DomainDataSourceList).ListMeta}
	for _, item := range obj.(*v1alpha1.DomainDataSourceList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested domainDataSources.
func (c *FakeDomainDataSources) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(domaindatasourcesResource, c.ns, opts))

}

// Create takes the representation of a domainDataSource and creates it.  Returns the server's representation of the domainDataSource, and an error, if there is any.
func (c *FakeDomainDataSources) Create(ctx context.Context, domainDataSource *v1alpha1.DomainDataSource, opts v1.CreateOptions) (result *v1alpha1.DomainDataSource, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(domaindatasourcesResource, c.ns, domainDataSource), &v1alpha1.DomainDataSource{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataSource), err
}

// Update takes the representation of a domainDataSource and updates it. Returns the server's representation of the domainDataSource, and an error, if there is any.
func (c *FakeDomainDataSources) Update(ctx context.Context, domainDataSource *v1alpha1.DomainDataSource, opts v1.UpdateOptions) (result *v1alpha1.DomainDataSource, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(domaindatasourcesResource, c.ns, domainDataSource), &v1alpha1.DomainDataSource{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataSource), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDomainDataSources) UpdateStatus(ctx context.Context, domainDataSource *v1alpha1.DomainDataSource, opts v1.UpdateOptions) (*v1alpha1.DomainDataSource, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(domaindatasourcesResource, "status", c.ns, domainDataSource), &v1alpha1.DomainDataSource{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataSource), err
}

// Delete takes name of the domainDataSource and deletes it. Returns an error if one occurs.
func (c *FakeDomainDataSources) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteActionWithOptions(domaindatasourcesResource, c.ns, name, opts), &v1alpha1.DomainDataSource{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDomainDataSources) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(domaindatasourcesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DomainDataSourceList{})
	return err
}

// Patch applies the patch and returns the patched domainDataSource.
func (c *FakeDomainDataSources) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DomainDataSource, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(domaindatasourcesResource, c.ns, name, pt, data, subresources...), &v1alpha1.DomainDataSource{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataSource), err
}
