// Copyright 2023 Ant Group Co., Ltd.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//   http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "github.com/secretflow/kuscia/pkg/crd/apis/kuscia/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDomainDataGrants implements DomainDataGrantInterface
type FakeDomainDataGrants struct {
	Fake *FakeKusciaV1alpha1
	ns   string
}

var domaindatagrantsResource = schema.GroupVersionResource{Group: "kuscia.secretflow", Version: "v1alpha1", Resource: "domaindatagrants"}

var domaindatagrantsKind = schema.GroupVersionKind{Group: "kuscia.secretflow", Version: "v1alpha1", Kind: "DomainDataGrant"}

// Get takes name of the domainDataGrant, and returns the corresponding domainDataGrant object, and an error if there is any.
func (c *FakeDomainDataGrants) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DomainDataGrant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(domaindatagrantsResource, c.ns, name), &v1alpha1.DomainDataGrant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataGrant), err
}

// List takes label and field selectors, and returns the list of DomainDataGrants that match those selectors.
func (c *FakeDomainDataGrants) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DomainDataGrantList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(domaindatagrantsResource, domaindatagrantsKind, c.ns, opts), &v1alpha1.DomainDataGrantList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DomainDataGrantList{ListMeta: obj.(*v1alpha1.DomainDataGrantList).ListMeta}
	for _, item := range obj.(*v1alpha1.DomainDataGrantList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested domainDataGrants.
func (c *FakeDomainDataGrants) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(domaindatagrantsResource, c.ns, opts))

}

// Create takes the representation of a domainDataGrant and creates it.  Returns the server's representation of the domainDataGrant, and an error, if there is any.
func (c *FakeDomainDataGrants) Create(ctx context.Context, domainDataGrant *v1alpha1.DomainDataGrant, opts v1.CreateOptions) (result *v1alpha1.DomainDataGrant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(domaindatagrantsResource, c.ns, domainDataGrant), &v1alpha1.DomainDataGrant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataGrant), err
}

// Update takes the representation of a domainDataGrant and updates it. Returns the server's representation of the domainDataGrant, and an error, if there is any.
func (c *FakeDomainDataGrants) Update(ctx context.Context, domainDataGrant *v1alpha1.DomainDataGrant, opts v1.UpdateOptions) (result *v1alpha1.DomainDataGrant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(domaindatagrantsResource, c.ns, domainDataGrant), &v1alpha1.DomainDataGrant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataGrant), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDomainDataGrants) UpdateStatus(ctx context.Context, domainDataGrant *v1alpha1.DomainDataGrant, opts v1.UpdateOptions) (*v1alpha1.DomainDataGrant, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(domaindatagrantsResource, "status", c.ns, domainDataGrant), &v1alpha1.DomainDataGrant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataGrant), err
}

// Delete takes name of the domainDataGrant and deletes it. Returns an error if one occurs.
func (c *FakeDomainDataGrants) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteActionWithOptions(domaindatagrantsResource, c.ns, name, opts), &v1alpha1.DomainDataGrant{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDomainDataGrants) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(domaindatagrantsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DomainDataGrantList{})
	return err
}

// Patch applies the patch and returns the patched domainDataGrant.
func (c *FakeDomainDataGrants) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DomainDataGrant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(domaindatagrantsResource, c.ns, name, pt, data, subresources...), &v1alpha1.DomainDataGrant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DomainDataGrant), err
}
